      ...
      <section className="auth">
        <form name={`${props.name}`} className="auth__form" onSubmit={props.onSubmit}>
          {path === "/signup" ? <>
            ...
            <input name="name" id="name" className="auth__input" type="text" defaultValue="Виталий" required />
          </> : <></>}
          <label htmlFor="email" className="auth__label">E-mail</label>
          <input name="email" id="email" className="auth__input" type="email" defaultValue="pochta@yandex.ru|" required />
          <label htmlFor="password" className="auth__label">Пароль</label>
          <input name="password" id="password" className="auth__input auth__input_pass" type="password" defaultValue="12345678901234" required />
